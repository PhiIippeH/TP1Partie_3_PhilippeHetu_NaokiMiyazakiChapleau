	...
	public void printFacture() {
		...
		for ( int i = 0; i < clients.size(); i++ ) {
			...
			for ( int j = 0; j < commandes.size(); j++ ) {
				...
				// On va cherche les 3 données, soit le nom du client, le plat,
				// et la quantité
				...
				if ( comSplit[0].equals( nom ) ) {
					...
					for ( int k = 0; k < plats.size(); k++ ) {
						...
						// On compare le nom du plat au nom du plat dans la
						// commande
						if ( platSplit[0].equals( comSplit[1] ) ) {

							// On rajoute au total l'équation qui est <quantité>
							// * <prix> du plat
			...
			// Il n'y a pas de moyen d'échouer cette étape. L'échouement de
			// cette étape
			...
			if ( !FactureZero( total ) ) {
				...
				temp += nom + " " + totalAvecTaxes[0] + "$\tTPS: " + totalAvecTaxes[1] + "$\tTVQ: " + totalAvecTaxes[2]
						+ "$\n";
				System.out.println( nom + " " + totalAvecTaxes[0] + "$\tTPS: " + totalAvecTaxes[1] + "$\tTVQ: "
						+ totalAvecTaxes[2] + "$\n" );
